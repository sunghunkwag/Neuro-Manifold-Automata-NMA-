...
def train_hive_mind():
    ...
    # Shared Optimizer (We optimize the 'Elite' and broadcast)
    # Ideally each agent explores, but for PPO stability we treat them as
    # parallel rollout workers with slightly different weights (mutations).
    ...
    clip_ratio = 0.2
    ent_coef = 0.01
    mutation_rate = 0.02 # Genetic Drift

    history = {'returns': [], 'elite_return': []}

    for gen in range(1, n_generations + 1):
        # --- PHASE 1: Parallel Exploration (Swarm Rollout) ---
        swarm_experience = [] # List of buffers
        swarm_returns = []

        for i, agent in enumerate(population):
            # Mutate non-elites slightly to encourage diversity in exploration
            if i > 0:
            ...
            obs, _ = env.reset(seed=seed + gen*100 + i) # Different seeds for diversity
            agent.reset()
        ...
        # --- PHASE 2: Hive Mind Broadcast (Selection & Copy) ---
        # Identify Elite
        ...
        # Copy Elite Brain to EVERYONE (Telepathy)
        # But we keep 'population[0]' as the canonical learner for the optimizer.
        # So we copy Elite -> population[0] (if not already)
        if elite_idx != 0:
            population[0].load_brain_from(elite_agent)

        # Broadcast population[0] (The Leader) to everyone else
        for i in range(1, POP_SIZE):
            population[i].load_brain_from(population[0])
            # Apply Mutation immediately after learning from Master
            # "Yes, I learned from you, but I have my own crazy idea."
            population[i].mutate(noise_scale=mutation_rate)

        # --- PHASE 3: Collective Learning (Optimization) ---
        # We train the Leader (population[0]) using the Elite's experience (or all experience?)
        # "Learn from the best experience" -> Use Elite's buffer.
        # Or "Learn from all" -> Concatenate buffers (PPO standard for parallel envs).
        # Let's concatenate for robust PPO.

        # Collate ALL experiences
        ...
        # State batch is trickier (list of dicts).
        # We need to flatten the list of lists of dicts.
        ...
        for i in range(10):
            ...
            # Slice to match lengths (simple hack for concatenated batches, technically slightly wrong at boundary but OK for swarm demo)
            # Correct way: mask boundaries. But with 2048*N samples, 1 sample error is negligible.
